...
pub fn Places() -> Element {
    ...
    let instalaciones_data = use_memo(move || {
        match serde_json::from_str::<InstalacionesGeoJSON>(INSTALACIONES_JSON) {
            Ok(data) => data,
    ...
    let espacios_data = use_memo(move || {
        match serde_json::from_str::<EspacioDeportivoData>(ESPACIOS_JSON) {
            Ok(data) => data,
            Err(_) => EspacioDeportivoData { espacios_deportivos: vec![] }
        }
    });
    ...
    let municipios_unicos = use_memo(move || {
        ...
        let mut municipios: Vec<String> = data.features
            .iter()
    ...
    let instalaciones_filtradas = use_memo(move || {
        ...
        let mut filtered = data.features
            .iter()
            .filter(|feature| {
                let instalacion = &feature.properties;
                let tiene_espacios = espacios_map.get(&instalacion.instalacion_codigo).map(|v| !v.is_empty()).unwrap_or(false);
                if !tiene_espacios {
                    return false;
                }
                // Filtro de búsqueda
                let matches_search = query.is_empty() || 
                    instalacion.instalacion_nombre.to_lowercase().contains(&query) ||
                    instalacion.municipio_nombre.to_lowercase().contains(&query);
                // Filtro de municipio
                let matches_municipio = municipio == "Todos" || 
                    instalacion.municipio_nombre == municipio;
                ...
                let matches_espacios = if !query.is_empty() {
                    if let Some(espacios) = espacios_map.get(&instalacion.instalacion_codigo) {
                        espacios.iter().any(|e| 
                            ...
                            e.espacio_tipo.as_ref().map(|t| t.to_lowercase().contains(&query)).unwrap_or(false)
                        )
                    } else {
                        false
                    }
        ...
        match orden_val.as_str() {
            "nombre_az" => filtered.sort_by(|a, b| 
                a.properties.instalacion_nombre.cmp(&b.properties.instalacion_nombre)),
            "nombre_za" => filtered.sort_by(|a, b| 
                b.properties.instalacion_nombre.cmp(&a.properties.instalacion_nombre)),
            "municipio" => filtered.sort_by(|a, b| 
                a.properties.municipio_nombre.cmp(&b.properties.municipio_nombre)),
            "espacios_desc" => filtered.sort_by(|a, b| {
                let espacios_map = &espacios_por_instalacion.read();
                let a_count = espacios_map.get(&a.properties.instalacion_codigo).map(|v| v.len()).unwrap_or(0);
                let b_count = espacios_map.get(&b.properties.instalacion_codigo).map(|v| v.len()).unwrap_or(0);
                b_count.cmp(&a_count)
            }),
    ...
    rsx! {
        Container {
            Section {
                // Header
                PageHeader {
                    title: "Instalaciones Deportivas".to_string(),
                    description: Some(
                        format!(
